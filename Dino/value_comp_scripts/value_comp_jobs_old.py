...
for interval_length in window_size:
    ...
    slug,mean_samples,data = cfu.value_comparison(slug,interval_length,num_samples,db_name='objective_cf_num',bandwidth=bandwidth,logger=None)
    results_w_param.append((slug,mean_samples,data,interval_length,bandwidth))
result_df = pd.DataFrame(results_w_param,columns=['slug','mean_samples','data','interval_length','bandwidth'])
outpath = 'val_comp2/result_df' + str(job_id) + '.pkl'
